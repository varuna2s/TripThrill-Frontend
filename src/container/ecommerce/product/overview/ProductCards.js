...
const ProductCards = ({ product }) => {
  const { id, name, city, rate, price, oldPrice, popular, img } = product;
  const dispatch = useDispatch();

  return (
    <ProductCard style={{ marginBottom: 30 }}>
      <figure>
        <img height="250px" src={require(`../../../../${img}`)} alt={`img${id}`} />
      </figure>
      <figcaption>
        ...
        <Heading className="product-single-title"  as="h5">
          <Link style={{color:"black",fontWeight:"800"}} to="/property1">{name}<div style={{fontSize:"14px", fontWeight:"200"}}>{city}</div>
          </Link>
        </Heading>
